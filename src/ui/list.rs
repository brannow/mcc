...
use crate::model::{
    EncodeJob, EncodeJobStatus, ProbeStatus, human_bitrate, human_duration, human_file_size,
};
...
impl VisibleColumns {
    fn from_width(width: u16) -> Self {
        ...
        Self {
            show: [true, true, w >= 55, w >= 70, w >= 85, w >= 100],
        }
    }
...
pub fn render_file_list(f: &mut Frame, app: &mut App, area: ratatui::layout::Rect, active: bool) {
    ...
    let all_headers = [
        (
            ...
            format!(
                " Name{}",
                sort_indicator(app.sort_column, SortColumn::Name, app.sort_ascending)
            ),
        ),
        (
            COL_CODEC,
            format!(
                "Codec{}",
                sort_indicator(app.sort_column, SortColumn::Codec, app.sort_ascending)
            ),
        ),
        (
            COL_SIZE,
            format!(
                "Size{}",
                sort_indicator(app.sort_column, SortColumn::Size, app.sort_ascending)
            ),
        ),
        (
            COL_BITRATE,
            format!(
                "Bitrate{}",
                sort_indicator(app.sort_column, SortColumn::Bitrate, app.sort_ascending)
            ),
        ),
        (
            COL_RESOLUTION,
            format!(
                "Res{}",
                sort_indicator(app.sort_column, SortColumn::Resolution, app.sort_ascending)
            ),
        ),
        (
            COL_DURATION,
            format!(
                "Duration{}",
                sort_indicator(app.sort_column, SortColumn::Duration, app.sort_ascending)
            ),
        ),
    ];
    ...
    let root_short = if root_display.len() > max_path_len && max_path_len > 4 {
        format!(
            "...{}",
            &root_display[root_display.len() - max_path_len + 3..]
        )
    } else {
...
fn build_media_row<'a>(
    ...
    let codec_cell = if let Some(job) = queue
        .iter()
        .find(|j| j.file_index == file_index && !j.status.is_finished())
    {
        match &job.status {
        ...
        match &file.probe_status {
            ProbeStatus::Pending | ProbeStatus::Probing => Cell::from(Span::styled(
                "···",
    ...
    let size_cell = Cell::from(
        Line::from(human_file_size(file.file_size)).alignment(ratatui::layout::Alignment::Right),
    )
...
fn build_folder_row<'a>(
    ...
    let name_cell = Cell::from(Line::from(vec![
        ...
        Span::styled(format!("  {} files", file_count), dim_style),
    ]));

    let empty = Cell::from("");

    let size_cell = Cell::from(
        Line::from(human_file_size(recursive_size)).alignment(ratatui::layout::Alignment::Right),
    )
